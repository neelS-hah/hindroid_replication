...
def helper(app_name, elem, num_blocks, def_dict, invoke_type):
    ...
    try:
        def_dict[app_name]['Packages'][package.group(1)].append(api_call)
    except:
        def_dict[app_name]['Packages'][package].append(api_call)

    def_dict[app_name]['Combined']['APIs'].append(api_call)
    def_dict[app_name]['invoke_type'][invoke_type].append(api_call)
    def_dict[app_name]['blocks'][tracking_number].append(api_call)
...
def create_struct(fp):
    ...
    num_blocks = 0
    tracking_number = 'method' + str(num_blocks)
    def_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    inside_block = False
    block_list = []
    smali_list = []
    ...
    for i in smali_list:
        ...
        for index, elem in enumerate(code):
            ...
            if flag and (api_list[0] in elem): 
                helper( app_name, elem, num_blocks, def_dict, api_list[0])
            if flag and (api_list[1] in elem):
                helper( app_name, elem, num_blocks, def_dict, api_list[1])
            if flag and (api_list[2] in elem): 
                helper(app_name, elem, num_blocks, def_dict, api_list[2])
            if flag and (api_list[3] in elem): 
                helper(app_name, elem, num_blocks, def_dict, api_list[3])
            if flag and (api_list[4] in elem):
                helper(app_name, elem, num_blocks, def_dict, api_list[4])
